...
func newMultiNamespaceCache(
	...
	globalConfig *cache.Config, // may be nil in which case no cache for cluster-scoped objects will be created
) cache.Cache {
...
func (c *multiNamespaceCache) GetInformerForKind(ctx context.Context, gvk schema.GroupVersionKind, opts ...cache.InformerGetOption) (cache.Informer, error) {
	// If the object is cluster scoped, get the informer from clusterCache,
	// if not use the namespaced caches.
...
func (c *multiNamespaceCache) Start(ctx context.Context) error {
	...
	for ns, c := range c.namespaceToCache {
		go func(ns string, c cache.Cache) {
